criaCartao(
    'Revolução Francesa',
    'Quais foram os principais nomes da revolução?',
    'Maximilien Robespierre, Georges Danton, Jean-Paul Marat'
)

criaCartao(
    'Revolução Francesa',
    'Quem fazia parte do 2°Estado antes da revolução?',
    '2°Estado: nobreza (rei, rainha e demais nobres)'
)

criaCartao(
    'Segunda Guerra Mundial',
    'Quando começou e terminou a Segunda Guerra Mundial?',
    'Iniciou em 1939 e terminou em 1945'
)

criaCartao(
    'Segunda Guerra Mundial',
    'Qual foi o estopim da Segunda Guerra Mundial?',
    'A invasão da Polônia pela Alemanha'
)

criaCartao(
    'Segunda Guerra Mundial',
    'Quais os nomes das alianças durante o conflito?',
    'Eixo e Aliados'
)

criaCartao(
    'História do Brasil',
    'Quando foi proclamada a Independência do Brasil?',
    'Em 1822'
)

criaCartao(
    'História do Brasil',
    'Quem foi o primeiro presidente do Brasil?',
    'Marechal Deodoro da Fonseca'
)

criaCartao(
    'História do Brasil',
    'Qual lei marcou o fim da escravidão no Brasil',
    'A Lei Áurea (1888)'
)